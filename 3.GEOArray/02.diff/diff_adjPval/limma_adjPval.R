...
library(limma)
library("impute")
setwd("C:\\Users\\YunGu\\Desktop\\diff")            #设定路径
rt=read.table("input.txt",sep="\t",header=T)
rt=as.matrix(rt)
rownames(rt)=rt[,1]
exp=rt[,2:ncol(rt)]
dimnames=list(rownames(exp),colnames(exp))
exp=matrix(as.numeric(as.matrix(exp)),nrow=nrow(exp),dimnames=dimnames)            #字符转换为数值

#impute missing expression data
mat=impute.knn(exp)
rt=mat$data

rt=avereps(rt)     #基因对应多个探针取均值，可以用nrow(rt)查看
#normalize
pdf(file="rawBox.pdf")
...
class <- c(rep("con",44),rep("treat",104))    #需要修改
